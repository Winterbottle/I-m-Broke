  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <form onSubmit={handleSearch} className="flex items-center gap-2 bg-white rounded-xl border border-brand-border shadow-sm p-2 mb-6 max-w-2xl">
        ...
        <button
          type="submit"
          className="px-4 py-2 bg-primary hover:bg-primary-dark text-white font-semibold rounded-lg transition-colors text-sm flex-shrink-0"
        >
          Search
        </button>
      </form>
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold">
          {selectedCategory ? `${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)} Deals` : 'Latest Deals'}
        </h2>
      ...
      {upcomingEvents.length > 0 && (
        <div className="mt-10">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/events" className="text-sm text-primary font-semibold hover:underline">
              View all →
            </Link>
          </div>
            ...
            {upcomingEvents.map((event) => (
              <div key={event.id} className="bg-white rounded-xl p-4 border border-brand-border hover:border-primary/30 transition-colors">
                <div className="flex gap-2 mb-2">
                  ...
                  {event.is_free && (
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-orange-50 text-primary">FREE</span>
                  )}
                </div>
